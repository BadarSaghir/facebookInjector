(() => {
  ...
  let $9ff52cf28abaa0d2$var$previousUrl;
  console.log("background.ts");
  $parcel$interopDefault($dBVaG).webNavigation.onHistoryStateUpdated.addListener((details => {
    const re = /(https:\/\/.*facebook.com\/groups\/treatearly\/?.*)|(https:\/\/.*facebook.com\/groups\/984165912191143\/?.*)|((https:\/\/.*facebook.com\/?)(home.php)?(?:\s|$))/gi;
    console.log(details.url.match(re)), details.url.match(re) && (console.log("url changed", details.url), 
    details.url != $9ff52cf28abaa0d2$var$previousUrl && (console.log(details, $9ff52cf28abaa0d2$var$previousUrl), 
    $parcel$interopDefault($dBVaG).tabs.reload(details.tabId, {
      bypassCache: !0
    }), $9ff52cf28abaa0d2$var$previousUrl = details.url));
  }));
})();